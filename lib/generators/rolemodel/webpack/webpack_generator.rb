module Rolemodel
  class WebpackGenerator < GeneratorBase
    ...
    # Optional coupling: when rolemodel:sentry is already installed, wire its
    # plugin into the webpack.config.js this generator just created. Declared
    # after the action methods so the config file exists before wiring runs.
    coupling_hook :sentry_webpack, with: :sentry
  end
end
